...
// Ambil semua data peserta (HANYA KETUA & INDIVIDU, BUKAN ANGGOTA)
$peserta_query = "SELECT 
                  ...
                  WHERE p.status_anggota != 'anggota'  -- HANYA tampilkan ketua & individu
...
<head>
    ...
    <style>
        ...
        .stats-card.verified .stats-icon {
...
<?php endif; ?>
        ...
        <div class="main-content">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted mb-0">Kelola dan monitor pendaftar event (ketua tim & individu)</p>
                </div>
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-3">
                    <div class="stats-card team">
                        ...
                        <p class="text-muted mb-0">Tim Terdaftar</p>
                    </div>
                </div>
            </div>
                                        ...
                                        <td>
                                            <?php if (!empty($peserta['nama_tim'])): ?>
                                                ...
                                                <div class="text-muted small">
                                                    <?php echo $peserta['kode_tim']; ?>
                                                    <?php if ($peserta['jumlah_anggota'] > 0): ?>
                                                        <span class="ms-2">
                                                            <i class="fas fa-user-friends"></i>
                                                            <?php echo $peserta['jumlah_anggota']; ?> anggota
                                                        </span>
                                                    <?php endif; ?>
                                                </div>
                                            <?php else: ?>
            ...
            <footer class="mt-4 text-center text-muted">
                ...
                <small>
                    ...
                    | Total Data: <?php echo mysqli_num_rows($peserta_result); ?> Pendaftar
                    | *Hanya menampilkan ketua tim dan peserta individu
                </small>
            </footer>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            // Cek apakah jQuery terload
            if (typeof $ === 'undefined') {
                console.error("jQuery tidak terload!");
                alert("Error: jQuery tidak terload. Silakan refresh halaman.");
                return;
            }
            
            // Cek apakah tabel ada
            if ($('#pesertaTable').length === 0) {
                console.error("Tabel #pesertaTable tidak ditemukan!");
                return;
            }
            ...
            try {
                ...
                dataTable.on('draw', function() {
                    updateStatistics();
                    $('#totalRecords').text(dataTable.rows({
                        search: 'applied'
                    }).count());
                });
                ...
                alert("Error menginisialisasi tabel: " + error.message);
            }
        });
        ...
        function updateStatistics() {
            try {
                const totalRows = dataTable.rows({
                    search: 'applied'
                }).count();
                ...
                error: function(xhr, status, error) {
                    ...
                    $('#detailContent').html(`
                        <div class="alert alert-danger">
                            ...
                            Gagal memuat data peserta: ${error}
                        </div>
                    `);
                }
            });
        ...
        // FUNGSI EDIT - Mengarahkan ke halaman edit
        function editPeserta(pesertaId) {
            window.location.href = 'edit_peserta.php?id=' + pesertaId;
        }

        // FUNGSI HAPUS - DIPERBAIKI
function deletePeserta(pesertaId, pesertaName) {
    console.log("=== DELETE FUNCTION STARTED ===");
    console.log("Peserta ID:", pesertaId);
    console.log("Peserta Name:", pesertaName);
    
    // Simpan event untuk referensi
    const deleteEvent = window.event || arguments.callee.caller.arguments[0];
    
    // Cek apakah pesertaId valid
    if (!pesertaId || pesertaId <= 0) {
        alert('Error: ID peserta tidak valid!');
        console.error("Invalid pesertaId:", pesertaId);
        return;
    }
    
    // Tampilkan konfirmasi sederhana dulu
    const userConfirmed = confirm(`Hapus peserta:\n\nNama: ${pesertaName}\nID: ${pesertaId}\n\nKlik OK untuk hapus, Cancel untuk batal.`);
    ...
    if (userConfirmed) {
        ...
        if (deleteEvent && deleteEvent.target) {
            deleteBtn = deleteEvent.target.closest('.btn-delete');
            if (deleteBtn) {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                if (xhr.status === 0) {
                    ...
                    errorMessage += 'File tidak ditemukan (404).';
        ...
        function printDetail() {
            try {
                ...
                alert('Gagal mencetak detail: ' + error.message);
            }
        }
